...
public class EnemyGun : MonoBehaviour
{
	...
	public float initialWait = 1.5f;					// The initial number of seconds before the weapon begins its normal shooting cycle
	...
	void Fire()
	{
		...
		for (int i = 0; i < shotPerRound; i++)
		{
			...
			endpoint = startSpot - (direction * 100000);		// The negative (subtraction) is used here since the enemies are facing the opposite direction as the player
			...
			if (Physics.Raycast(ray, out hit, range))
			{
				...
				else if (hit.collider.transform.parent != null)		// First make sure there is a parent
				{
